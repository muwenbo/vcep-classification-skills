# ClinGen Cardiomyopathy Expert Panel Variant Interpretation Guidelines for TNNT2
...
| **HGNC Name** | troponin T2, cardiac type |
| **Transcript** | NM_001276345.2 |
| **Disease** | Hypertrophic cardiomyopathy (MONDO:0005045); Dilated cardiomyopathy (MONDO:0005021) |
...
**Comments:** Not currently applicable to TNNT2. See PM4 for truncating variants that do NOT undergo NMD.
...
#### PS3 Strong - In Vitro Splicing Assays
...
#### PS3 Supporting - In Vitro Assays
...
#### Strength Thresholds (based on lower bound of 95% CI of OR)
...
| Strength | Criteria |
|----------|----------|
| **Supporting** | Applicable to missense variants in *TNNT2* in the specific regions listed below (Walsh et al. 2019). |

#### TNNT2 Cluster Regions

| Transcript | Codon Range |
|-----------|-------------|
| ENST00000367318 | Codons 79-179 |
| ENST00000656932.1 / NM_001276345.2 | Codons 89-189 |
...
2. The variant being seen in a single subpopulation meeting any of the following:
...
| Strength | Criteria |
|----------|----------|
| **Moderate** | A different missense variant at the same codon has been classified as **pathogenic** using these modified guidelines without application of PM5. The impact of the amino acid change being evaluated needs to be compared to the impact of the amino acid change that is established as pathogenic. Consider reducing the strength to SUPPORTING if the predicted impact is not expected to be equivalent or more severe. |
| **Supporting** | A different missense variant at the same codon has been classified as **likely pathogenic** using these modified guidelines without application of PM5. The impact of the amino acid change being evaluated needs to be compared to the impact of the amino acid change that is established as likely pathogenic. Consider reducing the strength to NOT APPLICABLE if the predicted impact is not expected to be equivalent or more severe. |
...
- If both PM5 and PM1 are applicable at SUPPORTING weight, use of PM5 is most appropriate since it is variant specific.
...
| Strength | Default Threshold | Highly Specific Phenotype Threshold |
|----------|-------------------|--------------------------------------|
...
> **Highly specific phenotype** is rare for inherited cardiomyopathies, but when the phenotype/presentation of a variant within and across families is highly specific (e.g., early-onset severe RCM in all affected individuals), the lower thresholds as proposed by Jarvik and Browning (2016) can be considered.
...
**Derivation:** The values used to calculate the BA1 threshold were derived from studies in Northern European populations that have been relatively well-characterized with regards to disease prevalence and variant spectrum. These thresholds can be applied to any population where disease prevalence is considered comparable (1/300 or lower).

**Application:** The threshold is applicable when assessing variants in the context of autosomal dominant cardiomyopathy.

**Database:** gnomAD is the preferred database for this calculation. If a subpopulation specific FAF other than the popmax is needed, this value can be calculated using the AlleleFrequencyApp on the CardioDB website:
...
Allele frequency is **≥0.0001** for **TNNT2** based on the **filtering allele frequency (FAF)** in **gnomAD** in the subpopulation with the highest frequency (popmax).
...
| Strength | Criteria |
|----------|----------|
| **Strong** | See PS3 specifications (splicing assays showing no effect). |
...
| **BP2** | Supporting | Other variants must be pathogenic as defined by these specifications. Testing of parents or other informative relatives is often required to determine *cis/trans* status. If a variant is seen in *trans* (or as double heterozygous) with another pathogenic variant in ≥2 cases and the phenotype is not more severe than when either variant is seen in isolation, this rule may be applied. <1% of cases of HCM have >1 P/LP variant (0.6%; Alfares et al. 2015). Cannot be applied when the variant has only been observed in *cis* with a pathogenic variant. Caution is needed if using this as a primary piece of evidence for LB/B classification. |
| **BP3** | *Not Applicable* | Not applicable to the current genes. |
| **BP4** | Supporting | Use of REVEL (Ioannidis et al. 2016) is recommended at thresholds of **≤0.40** for BP4. Clinical judgment is needed if any individual algorithms or conservation data are contradictory to REVEL data. SpliceAI is recommended for evaluation of predicted splice impacts. |
...
| 1 **Strong** (PS1, PS2, PS3, PS4, PP1_Strong) AND 2 **Moderate** (PS3_Moderate, PS4_Moderate, PM4, PM5, PM6, PP1_Moderate) AND ≥2 **Supporting** (PS3_Supporting, PS4_Supporting, PM1_Supporting, PM2_Supporting, PM5_Supporting, PP1, PP3) |
| 1 **Strong** (PS1, PS2, PS3, PS4, PP1_Strong) AND 1 **Moderate** (PS3_Moderate, PS4_Moderate, PM4, PM5, PM6, PP1_Moderate) AND ≥4 **Supporting** (PS3_Supporting, PS4_Supporting, PM1_Supporting, PM2_Supporting, PM5_Supporting, PP1, PP3) |
...
| 1 **Strong** (PS1, PS2, PS3, PS4, PP1_Strong) AND ≥2 **Supporting** (PS3_Supporting, PS4_Supporting, PM1_Supporting, PM2_Supporting, PM5_Supporting, PP1, PP3) |
| ≥3 **Moderate** (PS3_Moderate, PS4_Moderate, PM4, PM5, PM6, PP1_Moderate) |
| 2 **Moderate** (PS3_Moderate, PS4_Moderate, PM4, PM5, PM6, PP1_Moderate) AND ≥2 **Supporting** (PS3_Supporting, PS4_Supporting, PM1_Supporting, PM2_Supporting, PM5_Supporting, PP1, PP3) |
| 1 **Moderate** (PS3_Moderate, PS4_Moderate, PM4, PM5, PM6, PP1_Moderate) AND ≥4 **Supporting** (PS3_Supporting, PS4_Supporting, PM1_Supporting, PM2_Supporting, PM5_Supporting, PP1, PP3) |
...
### Appendix A: Criteria Applicability Summary for TNNT2
...
| PM2 | Yes | Supporting | Disease-specific |
| PM3 | No | N/A | N/A |
...
| BS1 | Yes | Strong | Disease-specific, Gene-specific |
| BS2 | No | N/A | N/A |
| BS3 | Yes | Strong | Disease-specific |
| BS4 | Yes | Strong | Disease-specific |
| BP1 | No | N/A | N/A |
| BP2 | Yes | Supporting | Disease-specific |
| BP3 | No | N/A | N/A |
| BP4 | Yes | Supporting | Disease-specific |
| BP5 | No | N/A | N/A |
| BP6 | No | N/A | N/A |
| BP7 | Yes | Supporting | General recommendation |

### Appendix B: Population Frequency Thresholds Summary

| Criterion | Threshold | Metric | Database | Strength |
|-----------|-----------|--------|----------|----------|
| BA1 | ≥0.001 | FAF (popmax) | gnomAD | Stand Alone |
| BS1 | ≥0.0001 | FAF (popmax) | gnomAD | Strong |
| PM2 | ≤0.00004 | Upper bound 95% CI (popmax) | gnomAD | Supporting |
...
| PP3/BP4 (splicing) | SpliceAI | As recommended | Splice impact |

### Appendix D: PM1 Cluster Regions for TNNT2

| Transcript | Codon Range | Basis |
|-----------|-------------|-------|
| ENST00000367318 | 79-179 | HCM case cohort enrichment (Walsh et al. 2019) |
| ENST00000656932.1 / NM_001276345.2 | 89-189 | HCM case cohort enrichment (Walsh et al. 2019) |
...
| 10 | Walsh R, Mazzarotto F et al. Quantitative approaches to variant classification increase the yield and precision of genetic testing in Mendelian diseases. *Genome Med* (2019) 11(1):5. | 30696458 |
| 11 | Kelly MA, Caleshu C et al. Adaptation and validation of the ACMG/AMP variant classification framework for MYH7-associated inherited cardiomyopathies. *Genet Med* (2018) 20(3):351-359. | 29300372 |
...
*This document was compiled from ClinGen Cardiomyopathy VCEP specifications and ClinGen SVI recommendations. For the most current version, please refer to the ClinGen website.*
