# ClinGen Cardiomyopathy Expert Panel Variant Interpretation Guidelines for ACTC1
...
**Comments:** Not currently applicable to ACTC1. See PM4 for truncating variants that do NOT undergo NMD.
...
**Cardiomyopathy-Specific Guidance:**
...
| **Supporting** | In vitro assays (e.g., biochemical assays of myofilament function, motility assays, human iPSC-CM) | Data from individual in vitro studies |
...
| Strength | Threshold (Lower Bound of 95% CI of OR) |
|----------|----------------------------------------|
| **Strong** | ≥20 |
| **Moderate** | ≥10 |
| **Supporting** | ≥5 |
...
**Note:** If multiple cohorts are available, the final ORs and associated CIs need to be harmonized across all cohorts to determine the final level (e.g., if 2 large cohorts have an OR of ~6 and a third small cohort has an OR of 11, application at a SUPPORTING level should be considered).
...
**Important:** HCM and DCM have distinct mechanisms of disease and therefore pathogenetic variants are not anticipated to cause both primary phenotypes. While occurrence in both phenotypes may initially be considered as evidence against pathogenicity, end-stage HCM can present similarly to DCM. Careful consideration is needed before including DCM or related phenotypes in case or segregation data for primarily HCM variants.
...
| Strength | Threshold |
|----------|-----------|
| **Supporting** | gnomAD popmax filtering allele frequency upper bound 95% CI **≤0.00004** |
...
Applying a threshold of ≤0.00004 (upper bound of 95% CI of the allele frequency in gnomAD) is equivalent to:

1. The variant NOT being observed more than once (≤1 allele) in gnomAD v.2.1.1 in one of the non-founder populations (e.g., absence required from the Other and Ashkenazi Jewish subpopulations)

2. The variant being seen in a single subpopulation and that subpopulation meets any of the following:
...
- The impact of the amino acid change being evaluated needs to be compared to the impact of the amino acid change that is established as pathogenic/likely pathogenic (e.g., a change of Ala to His is less severe than Ala to Cys change)
- Consider reducing the strength of this rule if the predicted impact is not expected to be equivalent or more severe
- **PM5 should not be combined with PM1.** If both are applicable at MODERATE weight, use of PM5 is most appropriate since it is variant specific. The one with the higher strength should be applied.
...
Due to the genotypic and phenotypic heterogeneity of inherited cardiomyopathies, conservative segregation thresholds have been set.

#### PP1 Thresholds (Standard)
...
#### PP1 Thresholds (Highly Specific Phenotype)
...
| Strength | Threshold |
|----------|-----------|
| **Stand Alone** | gnomAD popmax filtering allele frequency (FAF) **≥0.001** (0.1%) |
...
#### Calculating FAF for Specific Subpopulations
...
- Traditionally, once a variant is classified as Benign, it is rarely re-evaluated and so the highest confidence is needed to establish that classification on an allele frequency alone
...
| Strength | Threshold |
|----------|-----------|
| **Strong** | gnomAD popmax filtering allele frequency (FAF) **≥0.0001** (0.01%) for *ACTC1* |
...
| **BP2** | Supporting | Other variants must be pathogenic as defined by these specifications. If a variant is seen in *trans* (or as double heterozygous) with another pathogenic variant in ≥2 cases and the phenotype is not more severe than when either of the two variants are seen in isolation, this rule may be applied. (<1% of HCM cases have >1 P/LP variant; Alfares et al. 2015). Cannot be applied when variant has only been observed in *cis* with a pathogenic variant. |
| **BP3** | Not Applicable | Not applicable to the current genes. |
| **BP4** | Supporting | Use REVEL ≤0.40 threshold. Meta-predictors preferred over multiple individual predictors. SpliceAI recommended for splice impact evaluation. |
...
|-----------|-----------|--------|----------|
| **BA1** | ≥0.001 (0.1%) | gnomAD popmax FAF | Stand Alone |
| **BS1** | ≥0.0001 (0.01%) | gnomAD popmax FAF | Strong |
| **PM2** | ≤0.00004 | gnomAD popmax upper bound 95% CI | Supporting |
...
| **PP3** | REVEL | ≥0.70 |
| **BP4** | REVEL | ≤0.40 |
| **Splice predictions** | SpliceAI | Recommended for all splice impact evaluation |

### Appendix C: PS4 Case-Control Analysis Summary

| Strength | Lower Bound 95% CI of OR |
|----------|-------------------------|
| **Strong** | ≥20 |
| **Moderate** | ≥10 |
| **Supporting** | ≥5 |

**Calculator:** www.cardiodb.org

### Appendix D: Criteria Applicability Summary for ACTC1

| Criterion | Status |
|-----------|--------|
| PVS1 | Not Applicable |
| PS1 | Applicable (No change) |
| PS2 | Applicable (Disease-specific) |
| PS3 | Applicable (Disease-specific, multiple strengths) |
| PS4 | Applicable (Disease-specific, multiple strengths) |
| PM1 | Not Applicable |
| PM2 | Applicable (Supporting only) |
| PM3 | Not Applicable |
| PM4 | Applicable (General recommendation) |
| PM5 | Applicable (General recommendation, multiple strengths) |
| PM6 | Applicable (Disease-specific) |
| PP1 | Applicable (Disease-specific, multiple strengths) |
| PP2 | Not Applicable |
| PP3 | Applicable (Disease-specific) |
| PP4 | Not Applicable |
| PP5 | Not Applicable |
| BA1 | Applicable (Disease-specific) |
| BS1 | Applicable (Disease-specific, Gene-specific) |
| BS2 | Not Applicable |
| BS3 | Applicable (Disease-specific, multiple strengths) |
| BS4 | Applicable (Disease-specific) |
| BP1 | Not Applicable |
| BP2 | Applicable (Disease-specific) |
| BP3 | Not Applicable |
| BP4 | Applicable (Disease-specific) |
| BP5 | Not Applicable |
| BP6 | Not Applicable |
